...
describe x509_certificate('/etc/ssl/test.example.com.crt') do
  ...
  its(:subject) { should match(/CN\s*?=\s*?test.example.com/) }
  its(:issuer) { should match(/CN\s*?=\s*?Pebble Intermediate CA/) }
end
...
describe x509_certificate('/etc/ssl/new.example.com.crt') do
  ...
  its(:keylength) { should be 2048 }
  its(:validity_in_days) { should be > 30 }
  its(:subject) { should match(/CN\s*?=\s*?new.example.com/) }
  its(:issuer) { should match(/CN\s*?=\s*?Pebble Intermediate CA/) }
end
...
describe x509_certificate('/etc/ssl/4096.example.com.crt') do
  ...
  its(:validity_in_days) { should be > 30 }
  its(:subject) { should match(/CN\s*?=\s*?4096.example.com/) }
  its(:issuer) { should match(/CN\s*?=\s*?Pebble Intermediate CA/) }
end
